...
import { Decision, DecisionType, ActionRequest } from "../types";
...
function isValidHitlRequest(
  interrupt: Interrupt,
): interrupt is Interrupt & { value: { action_requests: ActionRequest[]; review_configs: any[] } } {
  return (
    !!interrupt.value &&
...
export function ThreadActionsView({
  ...
  const handleApproveAll = useCallback(() => {
    ...
    // Use chat.confirm to confirm execution
    try {
      chat.confirm();
      toast("Success", {
  ...
  }, [actionRequests, addressedActions, hasMultipleActions, chat]);
  ...
  const interruptValue = singleActionInterrupt.value as { action_requests: ActionRequest[]; review_configs: any[] };
  ...
  return (
    <div className="flex min-h-full w-full max-w-full flex-col gap-9">
      ...
      <InboxItemInput
        ...
        setSelectedSubmitType={(value) => {
          if (typeof value === 'function') {
            setSelectedSubmitType(value(selectedSubmitType));
          } else {
            setSelectedSubmitType(value);
          }
        }}
